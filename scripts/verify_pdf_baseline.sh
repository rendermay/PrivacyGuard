...
#
# 被守护的 PDF 端代码：
#   privacyguard/ocr/text_pdf.py         — PDF 文本通道
#   privacyguard/ocr/mixed_pdf.py        — PDF 混合通道（v39.6 仅 import，不修改）
#   privacyguard/workers/ocr_worker.py   — OCRWorker（PDF）— 含 page_result_signal payload
#
...
committed_changes="$(git diff --name-only "${REF}..HEAD" -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' 2>/dev/null)"
if [[ -n "$committed_changes" ]]; then
    echo "❌ FAIL [committed]: PDF-side files modified in commits since ${REF}:"
    echo "$committed_changes"
    git diff "${REF}..HEAD" -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' | head -30
    echo ""
    violations=$((violations + 1))
fi

# 2b) staged (--cached): 已 git add 但未 commit
staged_changes="$(git diff --name-only --cached -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' 2>/dev/null)"
if [[ -n "$staged_changes" ]]; then
    echo "❌ FAIL [staged]: PDF-side files staged for commit:"
    echo "$staged_changes"
    git diff --cached -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' | head -30
    echo ""
    violations=$((violations + 1))
fi

# 2c) working tree (unstaged): 已修改但未 git add
unstaged_changes="$(git diff --name-only -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' 2>/dev/null)"
if [[ -n "$unstaged_changes" ]]; then
    echo "❌ FAIL [working tree]: PDF-side files modified but not staged:"
    echo "$unstaged_changes"
    git diff -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' | head -30
    echo ""
    violations=$((violations + 1))
fi

# 3) 额外扫描：隐私目录中任何新增 / 重命名 / 删除的 PDF 端文件
new_paths="$(git diff --name-status "${REF}..HEAD" -- 'privacyguard/ocr/' 'privacyguard/workers/ocr_worker.py' 2>/dev/null | grep -E '^A|^D|^R' || true)"
if [[ -n "$new_paths" ]]; then
...
payload_changes="$( ( git diff "${REF}..HEAD" -- 'privacyguard/workers/ocr_worker.py'; git diff --cached -- 'privacyguard/workers/ocr_worker.py' ) 2>/dev/null | grep -E '^[+-].*page_result_signal' || true)"
