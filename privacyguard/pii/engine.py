...
WR-01 (02-04 acceptance): 本模块顶层导入 6 个新 validator 子模块（uscc / bank_card /
...
顶层导入。OPS-03 严格契约（`import privacyguard` 不触发任何 PII 模块加载）不受影响；
本模块顶层的 import 仅在 `import privacyguard.pii.engine` 时触发。各 validator 子模块
仍通过 `privacyguard.pii.validators.__init__._LAZY_IMPORTS` + `__getattr__` 提供独立的
延迟访问路径（用于命令行工具 / 测试 fixture / 单元测试等不需要 PIIEngine 的场景）。
如果未来需要更细粒度的按需加载，可在本模块内重构成 `importlib.import_module(...)`
形式；当前实现保留 eager 形态以保持代码简洁。
"""
import re
