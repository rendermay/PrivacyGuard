...
must_haves:
  ...
  artifacts:
    - privacyguard/pii/__init__.py (lazy _LAZY_IMPORTS + __getattr__ + __all__)
    - privacyguard/pii/hits.py (PIIHit dataclass frozen=True, 7 required fields D-05)
    - privacyguard/pii/validators/__init__.py (lazy re-export)
    - privacyguard/pii/validators/id_card.py (WEIGHTS, MAPPING, compute_check_digit, validate_18, validate_15, upgrade_15_to_18)
    ...
    - privacyguard/pii/normalize.py (normalize_digits, flatten_for_match, map_flat_to_original)
    - privacyguard/pii/confidence.py (ConfidenceTier Literal, classify_hit)
    - privacyguard/pii/mask.py (partial_mask_id_card, partial_mask_phone, mask_for_entity)
    - privacyguard/pii/overlap.py (resolve by page_offset+page_length dedup)
    - privacyguard/pii/engine.py (TextUnit dataclass, PIIEngine.detect pipeline)
    - privacyguard/pii/pdf_adapter.py (collect_pii_rects, apply_pii_redactions)
    ...
    - privacyguard/ocr/full_page_ocr.py (render_full_page_to_bgr, collect_full_page_ocr_hits with DI signature per D-03)
    ...
    - tests/unit/test_pii_validators.py (NUM-01/02/03 stubs)
    - tests/unit/test_pii_engine.py (ENGINE-01..07 stubs)
    - tests/unit/test_pdf_pii_redaction.py (reverse-extraction test)
    - tests/unit/test_pdf_pii_pipeline.py (end-to-end stub)
    - tests/unit/test_pii_offline.py (socket monkey-patch stub)
    - tests/unit/test_package_imports.py (extended with pii lazy-load assertion)
    - tests/unit/test_convergence.py (extended with no-inline-PIIHit-in-main.py assertion)
    - tests/unit/test_app_config.py (extended with pii_settings default + round-trip assertions)
    - config.json (pii_settings block added)
    - config.json.template (pii_settings block added)
    - privacyguard/__init__.py (_LAZY_IMPORTS extended with PIIEngine / PIIHit / validate_18_id / is_mobile_segment / TextUnit)
    - privacyguard/workers/ocr_worker.py (pii_signal signal, _detect_pii_for_page method)
    - main.py (page_data 'pii' key default, OCRWorker compat pii_signal wiring, _on_pii_page_result slot, SettingsDialog "5 隐私识别" tab + cb_pii_engine_enabled / cb_pii_auto_redact / cb_pii_require_confirm, SinglePageCanvas.paintEvent PII third loop, save_pdf pii_list merge)
    - packaging/windows/config/PrivacyGuard_windows.spec (datas append privacyguard/pii/data, hiddenimports append privacyguard.pii.*)
    - packaging/macos/scripts/build_complete.sh (data file copy verification)
  key_links:
...
<task type="tracer" tdd="true">
  ...
  <action>
    ...
    - Class `TestPdfPiiRedaction.test_image_pixels_destroyed`: builds a PDF with one ID card embedded inside a page-rendered image (simulate scanned PDF), applies redactions, re-extracts and asserts the ID card digits absent. Uses `fitz.PDF_REDACT_IMAGE_PIXELS = 2` explicitly; this guards Pitfall 9.
...
<task type="tracer" tdd="true">
  ...
  <action>
    ...
    **privacyguard/pii/hits.py** — `PIIHit` is `@dataclass(frozen=True)` with field order EXACTLY (D-05 lock): entity_type: str, page_offset: int, page_length: int, page_rect: Tuple[float, float, float, float], confidence_tier: str, source: str, mask_strategy: str. Add `normalized: str = ""` and `validator_passed: bool = True` with defaults. Use `Tuple` from typing (NOT `tuple` lowercase — keeps frozen hashability strict). Also define `TextUnit` dataclass (page_index: int, text: str, source: str) and `ConfidenceTier = Literal["HIGH","MEDIUM","LOW"]`.
