...
must_haves:
  truths:
    ...
    - `config.json` and `config.json.template` contain `pii_settings.per_entity_default` dict with all 9 keys (`CN_ID_CARD`, `CN_PHONE`, `CN_BANK_CARD`, `CN_EMAIL`, `CN_USCC`, `CN_TAXPAYER_ID`, `CN_TAXPAYER_ID_15`, `CN_VAT_INVOICE`, `CN_BANK_ACCOUNT`) — each default value `"partial"` (D-04 per_entity_default schema, config is the source of truth; runtime `mask_override_this_doc` is the in-memory override).
