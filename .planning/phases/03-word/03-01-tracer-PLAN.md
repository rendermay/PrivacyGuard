...
must_haves:
  truths:
    ...
    - "现有 79/79 测试基线保持通过（test_mixed_pdf_ocr / test_path_validation / test_ocr_api / test_package_imports / test_pdf_text_hit_dedup / test_app_config / test_word_replace_rules / test_batch_word_replace / test_config_alignment / test_fstring_safety / test_convergence）外加新增 tests/unit/test_word_pii_pipeline.py::TestWordPIIAutoTrigger / TestWordRedactRoundTrip / TestWordDocumentPropertiesCleared 三个测试类全部 green（per D-13 / D-14 / OPS-07）"
  artifacts:
    - privacyguard/word/__init__.py (_LAZY_IMPORTS + __getattr__ 懒加载入口)
    - privacyguard/word/adapter.py (WordAdapter.collect_units(docx_path) -> (List[TextUnit], Dict[int, str]))
    - privacyguard/word/worker.py (WordPIIWorker QThread + pii_signal / finished_signal / error_signal)
    - privacyguard/word/redact.py (redact_paragraph + redact_word wrapper 沿用 main.py:replace_matches_in_paragraph)
    - privacyguard/word/clear_doc_props.py (CORE_PROPS_TO_CLEAR + APP_PROPS_TO_CLEAR + clear_word_doc_props)
    - privacyguard/__init__.py MODIFY (_LAZY_IMPORTS 追加 WordAdapter / redact_word / clear_word_doc_props / WordPIIWorker 5 项符号)
    - tests/fixtures/__init__.py NEW (允许 tests/fixtures/* 被顶层 import 链发现)
    - tests/fixtures/fake_word.py NEW (build_fake_docx 合成含 PII 的 docx)
    - tests/unit/test_word_pii_pipeline.py NEW (3 个测试类 — TestWordAdapterCollectUnits / TestWordPIIAutoTrigger / TestWordRedactRoundTrip / TestWordDocumentPropertiesCleared / TestWordMergePriorityRulePiManualOcr)
    - main.py MODIFY (_open_word_docx 自动启动 WordPIIWorker / _on_word_pii_page_result 新槽 / merge_word_matches_with_priority 第六参数 / _save_word 接入 redact_word + clear_word_doc_props)
  key_links:
    ...
    - "merge_word_matches_with_priority 第六参数 pii_matches=None 默认值保持向后兼容（既有 79/79 测试 baseline 不破坏）（D-19）"
  prohibitions:
    ...
    - "不得让 _on_word_pii_page_result 写 word_data 后丢失 cp27 局部 patch 触发（Wave 2 实施；本波仅占位 emit 钩子）"
  backstop_statements:
...
threat_model:
  trust_boundaries:
    - name: docx file path input
      description: 用户拖拽 / Open 菜单传入 .docx 路径；main.py:_open_word_docx 解析 word/document.xml；WordAdapter.collect_units 遍历 paragraphs + tables；恶意 XML / 超大文档需走既有 validate_safe_path + TempFileManager
    - name: WordPIIWorker QThread → MainWindow 主线程
      description: pyqtSignal 跨线程传 (key, hits_dict_list)；主线程 _on_word_pii_page_result 反序列化为 PIIHit 列表；QMutexLocker 保护 word_data 写
    - name: redact_word wrapper → main.py 模块级函数
      description: redact_word 内部 import main.py:replace_matches_in_paragraph；隐私导入风险；必须用 lazy import inside function 避免 privacyguard.word 拉起 12.9k LOC main.py
    - name: clear_word_doc_props → python-docx core_properties / app_properties
      description: app_properties 在 python-docx v0.8.10 以下版本只读 / 不可用；需 hasattr + try/except 防御
    - name: PyInstaller frozen build → privacyguard.word.*
      description: 缺 hiddenimports 时 cp30 教训复现 ModuleNotFoundError;packaging/{windows,macos}/config/*.spec 必须字段级一致
  stride:
    - id: T-03-WordRunBoundary
      category: Information Disclosure
      component: privacyguard/word/redact.py::redact_word
      severity: high
      disposition: mitigate
      mitigation: 沿用 main.py:replace_matches_in_paragraph 既有 run-level 替换；tests/unit/test_word_pii_pipeline.py::TestWordRedactRoundTrip 构造 18 位身份证 + 调 redact_word + 反向断言 Document(out).paragraphs[*].text 不含原文（SAFE-02 reverse-extraction）
    - id: T-03-DocPropsLeak
      category: Information Disclosure
      component: privacyguard/word/clear_doc_props.py::clear_word_doc_props
      severity: high
      disposition: mitigate
      mitigation: 5 core 字符串字段 + revision=1 + 2 app 字段；tests/unit/test_word_pii_pipeline.py::TestWordDocumentPropertiesCleared 写含 PII 元数据 → clear → 反向断言 core_properties.title 等于 ""
    - id: T-03-PIIAutoTrigger
      category: Tampering / UX
      component: main.py::_open_word_docx
      severity: medium
      disposition: mitigate
      mitigation: WordPIIWorker.start() 紧接 word_data 初始化后调；tests/unit/test_word_pii_pipeline.py::TestWordPIIAutoTrigger 断言 worker run() 后 word_data[key][\"pii\"] 非空
    - id: T-03-MergePriority
      category: Tampering / Information Disclosure (false positive 误报)
      component: main.py:merge_word_matches_with_priority
      severity: medium
      disposition: mitigate
      mitigation: priority 锁定 rule > pii > manual > ocr；tests/unit/test_word_pii_pipeline.py::TestWordMergePriorityRulePiManualOcr 覆盖重叠区间 pii 覆盖 manual + rule 不被 pii 覆盖两种场景
    - id: T-03-LazyImport
      category: Denial of Service / OPS-03 regression
      component: privacyguard/word/__init__.py
      severity: medium
      disposition: mitigate
      mitigation: _LAZY_IMPORTS + __getattr__ 严格 lazy-load；tests/unit/test_package_imports.py::test_import_privacyguard_does_not_load_word_submodules 断言 sys.modules 中无 privacyguard.word.adapter / .worker / .redact / .clear_doc_props / .candidate_dialog
    - id: T-03-RealPiiInFixture
      category: Repudiation / OPS-05
      component: tests/fixtures/fake_word.py
      severity: high
      disposition: mitigate
      mitigation: build_fake_docx 仅调用 tests/fixtures/fake_pii.py 既有 Faker 合成器；test_word_pii_pipeline.py 末尾断言 fixture 不含真实身份字符串字面量
    - id: T-03-PyInstallerHiddenimports
      category: Denial of Service / cp30 教训扩展
      component: packaging/windows/config/PrivacyGuard_windows.spec + packaging/macos/config/PrivacyGuard.spec
      severity: high
      disposition: mitigate
      mitigation: 双 spec 字段级一致追加 5 项 privacyguard.word.* hiddenimports；tests/unit/test_package_imports.py 间接断言 frozen 启动可导入
    - id: T-03-RevisionType
      category: Information Disclosure (ValueError)
      component: privacyguard/word/clear_doc_props.py
      severity: medium
      disposition: mitigate
      mitigation: revision 字段单独 if hasattr(core,\"revision\") 分支处理为 int(1)；core 其他字符串字段置空字符串；app_properties 走 hasattr + try/except
    - id: T-03-WordDataRace
      category: Information Disclosure (race condition)
      component: WordPIIWorker + _save_word
      severity: medium
      disposition: mitigate
      mitigation: _on_word_pii_page_result 必须 QMutexLocker(self._word_data_lock) 写 word_data[key][\"pii\"]；_save_word 读 word_data 时同样加锁

---

<objective>
落地 Phase 3 端到端 spine：docx 打开 → 9 类 PII 自动识别（CN_ID_CARD / CN_PHONE / CN_BANK_CARD / CN_EMAIL / CN_USCC / CN_TAXPAYER_ID / CN_TAXPAYER_ID_15 / CN_VAT_INVOICE / CN_BANK_ACCOUNT）→ 写入 word_data[key]["pii"] 通道 → _save_word 调 redact_word 真脱敏 + clear_word_doc_props 清 8 字段 → reverse-extraction 断言 79/79 + 新增 3 测试类全 green。Wave 1 必须在最小可行切片上证明整条链路通；Wave 2 / 3 / 4 在此基础上扩展。
</objective>
...
Tracer 失败模式在最早期的、最小输入的提交点被捕获，而不是在 13 个文件都已被改完之后才发现。
</purpose>

<output>
- privacyguard/word/ 子包 5 个文件：__init__.py / adapter.py / worker.py / redact.py / clear_doc_props.py
- privacyguard/__init__.py MODIFY 追加 5 项 lazy forward
- tests/fixtures/fake_word.py + tests/fixtures/__init__.py
- tests/unit/test_word_pii_pipeline.py 5 个测试类（其中 3 个本波 green，2 个 Wave 2 落地）
- main.py 4 处修改：_open_word_docx 自动启动 + _on_word_pii_page_result 新槽 + merge_word_matches_with_priority 第六参数 + _save_word 扩 pii_matches + clear_word_doc_props 接线
</output>
...
<task type="tracer" tdd="true">
  <name>Wave 0 — 落地 privacyguard/word/ 子包骨架 + fake_word fixture + 测试 RED 基线</name>
  <files>
    - privacyguard/word/__init__.py
    - privacyguard/word/adapter.py
    - privacyguard/word/worker.py
  ...
  <read_first>
    ...
    - .planning/phases/03-word/03-RESEARCH.md (lines 1174-1240 — merge_word_matches_with_priority 扩展完整代码示例)
    - .planning/phases/03-word/03-RESEARCH.md (lines 1242-1304 — _on_word_pii_page_result + _apply_word_pii_panel_updates 完整代码示例)
  ...
  <action>
    Phase 3 Wave 1 = RED 基线任务。本任务**只写测试 + fixture + 模块骨架**（不含 runJavaScript 局部 patch — Wave 2 落地），不写真实业务逻辑；本任务结束时所有测试必须 RED（ModuleNotFoundError / ImportError），Wave 2 任务再写 GREEN 实现。
    ...
    **Step E — privacyguard/word/clear_doc_props.py 骨架**（NEW 文件）。模块 docstring "Phase 3 Word 文档属性清除（D-08 / D-24 — 与 Phase 2 SAFE-03 clear_pdf_metadata 对称）"。模块顶层 Final 常量：CORE_PROPS_TO_CLEAR: Final = ('title', 'author', 'subject', 'keywords', 'last_modified_by')（5 字段字符串）+ APP_PROPS_TO_CLEAR: Final = ('company', 'manager')（2 字段字符串）。定义 def clear_word_doc_props(doc) -> None：函数体 NotImplementedError. __all__ = ['clear_word_doc_props', 'CORE_PROPS_TO_CLEAR', 'APP_PROPS_TO_CLEAR']。

    **Step F — privacyguard/__init__.py _LAZY_IMPORTS 扩展**（MODIFY）。在文件末尾 _LAZY_IMPORTS 字典追加 5 项：'WordAdapter': ('privacyguard.word', 'WordAdapter') / 'redact_word': ('privacyguard.word', 'redact_word') / 'clear_word_doc_props': ('privacyguard.word', 'clear_word_doc_props') / 'WordPIIWorker': ('privacyguard.word', 'WordPIIWorker') / 'WordCandidateDialog': ('privacyguard.word', 'WordCandidateDialog')。同时在 __all__ 列表同步追加这 5 个字符串。**不**修改文件中其他任何代码（保持 Phase 2 既有 33 项符号与 _LAZY_IMPORTS 不动）。

    **Step G — tests/fixtures/__init__.py NEW**（允许 tests/fixtures/fake_word 通过 tests.fixtures.fake_word 路径被 unittest 发现）。文件为空 docstring "测试夹具包入口（Phase 3 — fake_word 合成器）"。

    **Step H — tests/fixtures/fake_word.py build_fake_docx 完整实现**（NEW 文件）。模块 docstring "Phase 3 合成含 PII 的 docx fixture（D-26 / OPS-05 — Faker 合成数据）"。实现 def build_fake_docx(paragraphs: Optional[List[str]] = None, tables: Optional[List[List[List[str]]]] = None, add_pii: bool = True) -> str。函数体严格按 03-RESEARCH.md:1316-1362 示例：from docx import Document + from typing import List, Optional；doc = Document()；遍历 paragraphs 调 doc.add_paragraph(p_text)；遍历 tables 三层循环（r_idx / c_idx）调 table.rows[r_idx].cells[c_idx].text = cell_text（与 _open_word_docx:10807-10819 表格初始化路径对齐）；add_pii=True 时 from tests.fixtures.fake_pii import fake_id_card / fake_phone / fake_email / fake_bank_card / fake_uscc 各追加一个 PII 段落（"甲方身份证 {fake_id_card()}" / "联系电话 {fake_phone()}" / "邮箱 {fake_email()}" / "卡号 {fake_bank_card()}" / "统一信用代码 {fake_uscc()}"）；最后 import tempfile + os；fd, path = tempfile.mkstemp(suffix='.docx')；os.close(fd)；doc.save(path)；return path。__all__ = ['build_fake_docx']。

    **Step I — tests/unit/test_word_pii_pipeline.py 5 个测试类 RED 骨架**（NEW 文件）。模块 docstring "Phase 3 Word 端到端 PII 流程测试（D-13 — 79/79 升级 88/88）"。import：unittest / os / tempfile / from docx import Document / from privacyguard.pii.engine import PIIEngine / from privacyguard.pii.hits import TextUnit, PIIHit / from privacyguard.pii.mask import mask_for_entity / from tests.fixtures.fake_pii import fake_id_card, fake_phone, fake_email, fake_bank_card, fake_uscc, fake_vat_invoice_20, fake_bank_account / from tests.fixtures.fake_word import build_fake_docx。本任务实现以下 5 个测试类的方法签名（函数体用 self.fail 或 assert False 标 RED）：

    TestWordAdapterCollectUnits.test_collect_units_returns_text_unit_per_block：调 build_fake_docx(paragraphs=["段落 0", "段落 1"]) → 调 WordAdapter.collect_units(path) → 断言 len(units) >= 2 且 key_index[0] == "paragraph_0" 且 key_index[1] == "paragraph_1"。本任务标 RED（NotImplementedError）。

    TestWordPIIAutoTrigger.test_engine_detects_pii_in_word_text：构造 text = f"身份证 {fake_id_card()} 手机 {fake_phone()} 邮箱 {fake_email()} 卡号 {fake_bank_card()} USCC {fake_uscc()} 发票 {fake_vat_invoice_20()} 账号 {fake_bank_account()}" → engine = PIIEngine() → hits = engine.detect(TextUnit(page_index=0, text=text, source='text')) → entity_types = {h.entity_type for h in hits} → 断言 len(entity_types) >= 6 且 for et in ('CN_ID_CARD','CN_PHONE','CN_EMAIL','CN_BANK_CARD','CN_USCC') self.assertIn(et, entity_types)。本任务标 RED（fake_vat_invoice_20 / fake_bank_account 可能未导入 — 见依赖）。注意：fake_vat_invoice_20 与 fake_bank_account 已在 tests/fixtures/fake_pii.py:165-181 落地（已在 Phase 2 02-01 实现），可直接 import。

    TestWordRedactRoundTrip.test_redact_word_partial_mask_visible：从 main import merge_word_matches_with_priority / from privacyguard.word.redact import redact_word / from privacyguard.word.clear_doc_props import clear_word_doc_props。secret_id = fake_id_card() → path = build_fake_docx(paragraphs=[f"原文 {secret_id}"]) → 模拟 word_data 初始化为 word_data = {"paragraph_0": {"text": f"原文 {secret_id}", "ocr": [], "manual": [], "pii": []}} → engine = PIIEngine() → hits = engine.detect(TextUnit(page_index=0, text=word_data["paragraph_0"]["text"], source='text')) → word_data["paragraph_0"]["pii"] = hits → doc = Document(path) → merged = merge_word_matches_with_priority(word_data["paragraph_0"]["text"], rules=[], default_replacement_text="[已脱敏]", manual_matches=[], ocr_matches=[], pii_matches=hits) → redact_word(doc, "paragraph_0", merged) → clear_word_doc_props(doc) → tempfile.NamedTemporaryFile 写 out_path → doc.save(out_path) → out_doc = Document(out_path) → out_text = "".join(p.text for p in out_doc.paragraphs) → 断言 self.assertNotIn(secret_id, out_text) 且 self.assertIn(mask_for_entity("CN_ID_CARD", secret_id), out_text)。本任务标 RED（merge_word_matches_with_priority 第六参数未就位 / redact_word / clear_word_doc_props NotImplementedError）。

    TestWordDocumentPropertiesCleared 含 2 个方法：(a) test_clear_core_5_fields_always_succeeds：path = build_fake_docx(paragraphs=["test"]) → doc = Document(path) → doc.core_properties.title = "敏感标题" / author = "敏感作者" / subject = "敏感主题" / keywords = "敏感关键字" / last_modified_by = "敏感修改者" → clear_word_doc_props(doc) → 写 out_path → out_doc = Document(out_path) → 断言 5 字段全部 == ""。本任务标 RED。(b) test_clear_revision_set_to_1：path = build_fake_docx(paragraphs=["test"]) → doc = Document(path) → doc.core_properties.revision = 99 → clear_word_doc_props(doc) → 断言 doc.core_properties.revision == 1。本任务标 RED。

    TestWordMergePriorityRulePiManualOcr 含 2 个方法：(a) test_rule_beats_pii：text = "张三 53010219200508011X" / secret_id = "53010219200508011X" / hit = PIIHit(entity_type='CN_ID_CARD', page_offset=3, page_length=18, page_rect=(0,0,0,0), confidence_tier='HIGH', source='text', mask_strategy=mask_for_entity('CN_ID_CARD', secret_id), normalized=secret_id) / rules = [{"enabled": True, "mode": "exact", "find": "张三", "replace": "[姓名]"}] / merged = merge_word_matches_with_priority(text, rules, "[已脱敏]", manual_matches=[], ocr_matches=[], pii_matches=[hit]) → 断言 len(merged) == 2 且 merged[0]["source"] == "rule" 且 merged[0]["replacement"] == "[姓名]" 且 merged[1]["source"] == "pii"。本任务标 RED。(b) test_pii_beats_manual_on_overlap：text = "53010219200508011X" / 同样构造 hit / manual = [{"start":0,"end":18,"text":secret_id,"replacement":"[手动]"}] / merged = merge_word_matches_with_priority(text, [], "[已脱敏]", manual_matches=manual, ocr_matches=[], pii_matches=[hit]) → 断言 len(merged) == 1 且 merged[0]["source"] == "pii"。本任务标 RED。

    **Step J — main.py 4 处修改**（MODIFY）。本任务**仅做骨架占位**，不写真实业务，确保 RED 状态：

    (a) main.py:10777 _open_word_docx 内 word_data 初始化完成后（紧接 self._reset_batch_session_state() 与 self.word_compare_mode = False 之间 — 见 main.py:10787 附近），插入以下骨架代码（仅占位，不实际启动 worker — Wave 2 实施）：
    ```python
    # Phase 3 (03-word) — Wave 1 RED 占位 — Wave 2 实施 WordPIIWorker 启动
    self._word_pii_worker = None
    ```

    (b) main.py 在 _on_pii_page_result 后（约 main.py:11521 后）新增 _on_word_pii_page_result 占位方法（Wave 1 仅函数签名，函数体 NotImplementedError 或 self.fail 占位）：
    ```python
    def _on_word_pii_page_result(self, key: str, hits_data: list):
        """Phase 3: pii_signal 接收槽（D-09 / D-18 — Wave 1 占位，Wave 2 实施）"""
        raise NotImplementedError("Wave 2 实施：QMutexLocker 写 word_data[key]['pii'] + _apply_word_pii_panel_updates")
    ```

    (c) main.py:863 merge_word_matches_with_priority 函数签名扩展 — 第六参数 pii_matches=None（默认值保持 back-compat），函数体**保持现状**（不引入 pii 优先级处理 — Wave 2 实施）。扩展后签名：`def merge_word_matches_with_priority(text, rules, default_replacement_text, manual_matches=None, ocr_matches=None, pii_matches=None):`；在函数体 _append_candidates(ocr_matches, "ocr") 后追加一行 `_append_candidates(pii_matches, "pii")`（Wave 1 RED 状态因为 _append_candidates 还没适配 PIIHit dataclass 实例 — 会触发 KeyError 或 start/end AttributeError；Wave 2 修复）。**重要**：保持 priority 顺序为 `_append_candidates(build_word_rule_matches(...), "rule")` → `_append_candidates(pii_matches, "pii")` → `_append_candidates(manual_matches, "manual")` → `_append_candidates(ocr_matches, "ocr")`（D-19 锁定 — pii 比 manual 高优先级）。

    (d) main.py:12699 _save_word 内 paragraphs 循环 + tables 循环（main.py:12715-12766）merge_word_matches_with_priority 调用追加 pii_matches 入参 — 调用形态：
    ```python
    merged_matches = merge_word_matches_with_priority(
        ...
        pii_matches=data.get("pii", []),  # Phase 3 NEW — Wave 1 RED 占位（Word data['pii'] 暂时为空 list，行为不变）
    )
    ```
    同时把 `replace_matches_in_paragraph(para, merged_matches, text_offset=0, fallback_replacement_text=self.replacement_text)` 这一调用改为调 `redact_word(new_doc, key, merged_matches, self.replacement_text)`（Wave 1 RED 状态：redact_word 抛 NotImplementedError；Wave 2 实施后正常）。tables 循环内 cell 多段循环也改为 redact_word 调用。**不**改 _save_word 的 try/except 结构。在 `new_doc.save(fname)`（main.py:12769）**之前**插入以下占位：`from privacyguard.word.clear_doc_props import clear_word_doc_props` + `clear_word_doc_props(new_doc)`（Wave 1 RED 状态抛 NotImplementedError；Wave 2 实施）。

    **Step K — 验证 RED 基线**。运行命令 `python3 -m compileall -q main.py privacyguard tests` 期望 exit 0（语法 green）。运行 `python3 -m unittest tests.unit.test_word_pii_pipeline -v 2>&1 | tail -20` 期望看到 5 个测试类全部以 ERROR（NotImplementedError）或 ImportError 形式报告 — RED 状态确认。运行 `python3 -m unittest tests.unit.test_word_replace_rules tests.unit.test_batch_word_replace tests.unit.test_convergence -v` 期望既有 79/79 基线保持 green（merge_word_matches_with_priority 第六参数默认值 back-compat 验证）。
  </action>
  <verify>
    <automated>python3 -m compileall -q main.py privacyguard tests && python3 -m unittest tests.unit.test_word_pii_pipeline -v 2>&1 | tail -20</automated>
  </verify>
  <acceptance_criteria>
  ...
  <done>
    privacyguard/word/ 5 个骨架文件就位（adapter / worker / redact / clear_doc_props 占位 NotImplementedError；__init__.py _LAZY_IMPORTS 完整）；privacyguard/__init__.py 5 项 lazy forward 就位；tests/fixtures/fake_word.py build_fake_docx 完整实现；tests/unit/test_word_pii_pipeline.py 5 个测试类 7 个测试方法 RED 骨架就位；main.py 4 处修改骨架就位（_open_word_docx 占位 / _on_word_pii_page_result 占位 / merge_word_matches_with_priority 第六参数 + _append_candidates(pii_matches, "pii") 占位 / _save_word pii_matches 入参 + redact_word + clear_word_doc_props 占位）；所有 RED 测试失败原因可定位到具体 NotImplementedError 或 AttributeError。
  </done>
  <reversibility>rating="reversible" rationale="占位骨架 + fixture + 测试；Wave 2/3/4 在此基础扩展，删除 RED 测试 + 占位即可恢复 v37.7.6 状态。"</reversibility>
</task>
...
<task type="auto" tdd="true">
  ...
  <acceptance_criteria>
    ...
    - TestWordRedactRoundTrip.test_redact_word_partial_mask_visible 断言 out_text 不含 secret_id 且含 mask_for_entity("CN_ID_CARD", secret_id) 字符串（reverse-extraction SAFE-02 + MASK-01 partial mask 落地）。
    ...
    - TestWordMergePriorityRulePiManualOcr.test_rule_beats_pii 断言 merged[0]["source"] == "rule" 且 merged[1]["source"] == "pii"（D-19 priority 锁 — rule 比 pii 高优先级且不重叠）。
    ...
    - `python3 -m unittest tests.unit.test_word_replace_rules tests.unit.test_batch_word_replace tests.unit.test_convergence tests.unit.test_package_imports tests.unit.test_app_config tests.unit.test_mixed_pdf_ocr tests.test_path_validation tests.unit.test_ocr_api tests.unit.test_pdf_text_hit_dedup tests.unit.test_config_alignment tests.unit.test_fstring_safety -v`（79/79 既有基线）全部 green — merge_word_matches_with_priority 第六参数 back-compat 验证。
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
    在 Wave 1 GREEN 测试全 pass 之后，请求人工在真实 UI 中打开含 PII 的 docx 文件以验证三个 Phase 3 关键行为在真实 PyQt6 环境下不回归。

    人工验证步骤：
    1. 启动应用：`cd /mnt/g/Project/PrivacyGuard && python3 main.py`
    2. 构造含 PII 的 docx：`python3 -c "from tests.fixtures.fake_word import build_fake_docx; p = build_fake_docx(); print(p)"`（输出路径用作 Open 菜单输入）
    3. 主菜单 → Open → 选择上面合成的 docx（路径需复制）
    4. 观察：
       a. 状态栏底部出现 `wordPiiStatusChip` 文本依次 `正在抽出 Word 段落文本…` → `扫描 Word 文本层…` → `扫描完成：未发现敏感内容` 或 `已识别 N 项敏感内容`（Phase 3 status chip 7 阶段 — Wave 1 仅占位 emit，UI 文案可能临时简化；Wave 2 完整实施）
       b. info_bar 中 wordPiiStatusChip 与 Phase 1 `piiStatusChip`（PDF）共存且无样式冲突
    5. 切换到对比模式（菜单或工具栏触发）—— Wave 1 阶段：左栏仅显示原文（**不**含 PII `<mark>` 高亮 — Wave 2 实施 cp27 局部 patch）；右栏仅显示原文（**不**含 partial mask — Wave 2 实施）。**期望**：UI 流程不卡死、不抛异常、不破坏 Phase 1/2 既有双栏对比预览的滚动 / 缩放行为
    6. 保存（Ctrl+S）→ 选择输出路径 → 等待保存完成 → 弹出"文件已保存至：{fname}"对话框
    7. 在文件管理器或 terminal 用 `python3 -c "from docx import Document; d = Document('{fname}'); print('title=', repr(d.core_properties.title), 'author=', repr(d.core_properties.author))"` 验证 5 core 字段全部空字符串
    8. `unzip -p {fname} docProps/core.xml | head -20` 应看不到原始敏感 title / author 字符串
  <what-built>
    Wave 1 GREEN 任务实施完成后：privacyguard/word/ 子包 5 模块（adapter / worker / redact / clear_doc_props）落地；main.py 4 处接线（_open_word_docx 自动启动 WordPIIWorker / _on_word_pii_page_result 槽 / merge_word_matches_with_priority 第六参数 pii_matches / _save_word 扩 redact_word + clear_word_doc_props）；tests/unit/test_word_pii_pipeline.py 5 个测试类 7 个测试方法 GREEN；79/79 既有基线保持 GREEN。
  </what-built>
  <how-to-verify>
  ...
  <action>
    阻塞型 checkpoint：等待用户回复 "approved" 或失败步骤与异常信息。Wave 1 GREEN 任务已完成 5 个 privacyguard/word/ 子模块落地 + main.py 4 处接线 + 5 个测试类 GREEN + 79/79 既有基线保持。本任务仅观察 UI 行为，不修改代码。
  </action>
...
|----------|-------------|
| docx file path input | 用户拖拽 / Open 菜单传入 .docx 路径；main.py:_open_word_docx 解析 word/document.xml；恶意 XML / 超大文档走既有 validate_safe_path + TempFileManager 防御 |
...
| T-03-PyInstallerHiddenimports | Denial of Service / cp30 | packaging/{windows,macos}/config/*.spec | high | mitigate | 双 spec 字段级一致追加 5 项 privacyguard.word.* hiddenimports；Wave 4 落地 |
...
python3 -m compileall -q main.py privacyguard tests \
  && python3 -m unittest \
      ...
      -v
```
期望全部 OK；tests.unit.test_word_pii_pipeline 7 个测试方法全 green，11 个基线模块保持 green（基线从 79/79 升级到 86/86 — Wave 4 升级到 88/88）。
...
- [ ] 真实 PyQt6 UI 验证通过（人工 checkpoint 9 步骤）
</success_criteria>
